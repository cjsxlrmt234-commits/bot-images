...
const FARM_TABLE = [
  ...
  ['jackpot', 6.0],      // 잭팟: 6%
  ['damage', 45.0],      // 데미지: 45%
  ['kill_single', 30.0], // 단일 교전: 30%
  ['kill_multi', 15.0]   // 다중 교전: 15%
];

const ESCAPE_TABLE = [
  ['instant_heal', 50.0], // 50%: 20~30 HP 즉시 회복
  ['drink', 30.0],        // 30%: 에너지 드링크 (2턴 동안 5씩 회복)
  ['painkiller', 20.0]    // 20%: 진통제 (3턴 동안 5씩 회복)
];
...
// 강화 단계별 세부 스탯(배율, 머리/몸/다리 확률) 계산 함수
...
function calculatePartDamage(profile) {
  ...
  if (roll < stats.numHead) {
    ...
    // 최소 100 데미지 보장 + 전투력 기반 피해량 + 강화 단계별 가중치
...
function resolveFarmFight(profile, battle) {
  ...
  switch (outcome) {
    ...
    case 'damage': {
      ...
      mainText = `⚠️ ${targetName}의 사격을 받아 기습당했습니다!\n💔 HP -${finalDamage}${reduceMsg}${notes}`;
      break;
    }
    case 'kill_single': {
      isDamageEvent = true;
  ...
  switch (outcome) {
    ...
    case 'drink': {
      if (activeBuffCount > 0) {
        ...
        textResult = `🧪 [에너지 드링크] 발동 (2턴간 매턴 HP +5 회복)`;
      }
      break;
    }
    case 'painkiller': {
      if (activeBuffCount > 0) {
        ...
        textResult = `💊 [진통제] 발동 (3턴간 매턴 HP +5 회복)`;
      }
      break;
    }
    default: {
...
function processGoldEnhance(profile, count = 1) {
  ...
  return { text: `⚡ 전투력 레벨 +${count} 업그레이드 완료! (현재 Lv.${profile.combatLevel})`, imageUrl: getEnhanceImage('success', profile.enhance) };
}
...
function processTurn(state, utterance) {
  ...
  if (utterance === '/4655') {
    ...
    return { text: `🎁 [시크릿 코드 적용]\n현금 10,000,000원이 지급되었습니다!${board}`, choices: isPlayingBattle ? BATTLE_CHOICES : LOBBY_CHOICES, category: 'secret', imageUrl: getEnhanceImage('success', profile.enhance) };
  }
