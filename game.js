...
function processImprintReroll(profile) {
  ...
  if (profile.cash < costCash || (profile.gold || 0) < costGold) {
    return { 
      text: `⚠️ 각인 변경 재화가 부족합니다!\n(필요: ${won(costCash)}, 금괴 ${costGold}개 [잠금슬롯 ${lockedCount}개 적용])\n(보유: ${won(profile.cash)}, 금괴 ${(profile.gold || 0).toLocaleString()}개)` 
    };
  }
  ...
  unlockedKeys.forEach(k => {
    if (profile.imprintLocks[k]) {
      const existingOpt = profile.imprints[k].options[0];
      results.push(`• 각인 ${k} (잠금 유지) : ${existingOpt.name} +${existingOpt.value}${existingOpt.unit}`);
    } else {
      let pool = [...IMPRINT_OPTION_POOL];
      const idx = rand(0, pool.length - 1);
      const optTemplate = pool[idx];
      const val = pickWeightedValue(optTemplate.values, optTemplate.weights);
      const newOpt = { name: optTemplate.name, value: val, unit: optTemplate.unit, key: optTemplate.key };
      
      profile.imprints[k] = { options: [newOpt] };
      results.push(`• 각인 ${k} (변경됨) : ${newOpt.name} +${newOpt.value}${newOpt.unit}`);
    }
  });
  ...
  return {
    text: [
      `🔮 [각인 변경 완료]`,
      `(소모 비용: ${won(costCash)}, 금괴 ${costGold}개 | 잠금 슬롯: ${lockedCount}개)`,
      ``,
      results.join('\n'),
      ``,
      resourceText(profile)
