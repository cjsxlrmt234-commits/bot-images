...
function addExp(profile, baseAmount) {
  ...
  const finalAmount = Math.round(
    baseAmount * getExpMultiplier(profile)
  );
  ...
  while (profile.exp >= getRequiredExp(profile.level)) {
    ...
    levelUpMessages.push(
      `🎉 [LEVEL UP!] Lv.${profile.level} 달성!`
    );
  }

  return {
    leveledUp: levelUpMessages.length > 0,
    msg: levelUpMessages.join('\n'),
    gained: finalAmount
  };
}
...
function createBattle(profile) {
  ...
  if (matchRoll >= 85 && matchRoll < 95) {
    mode = '듀오';
  } else if (matchRoll >= 95) {
    mode = '스쿼드';
  }
  ...
  return {
    ...
    startSnapshot: {
      ...
      gold: profile?.gold || 0,
      keys: profile?.keys || 0,
      monthItems: profile?.monthItems || 0
    },
  };
}

function processBuffs(battle) {
  if (!battle || !battle.buffs || battle.buffs.length === 0) {
    return [];
  }
  ...
  for (let i = battle.buffs.length - 1; i >= 0; i--) {
    ...
    if (buff.turnsLeft > 0) {
      ...
      battle.hp = Math.min(
        100,
        battle.hp + buff.healAmount
      );

      buff.turnsLeft -= 1;

      buffMessages.push(
        `✨ ${buff.name} 효과! HP +${heal} 회복 (남은 효과: ${buff.turnsLeft}턴)`
      );
    }

    if (buff.turnsLeft <= 0) {
      buffMessages.push(
        `✨ ${buff.name} 지속 시간이 종료되었습니다.`
      );
...
function battleStatusBoard(profile, battle) {
  ...
  let boardLines = [
    `----------------------------------------`,
    `⚔️  배틀로얄 진행 중  [ 매칭: ${b.mode} ]`,
    `----------------------------------------`,
    `⏱️ 턴    : ${b.turn} / ${b.maxTurn} 턴`,
    `👥 생존자 : ${b.survivors}명`,
    `❤️  HP    : ${makeHpBar(b.hp)}`,
  ...
  if (b.buffs.length > 0) {
    const buffDesc = b.buffs
      .map(buff => `${buff.name}(${buff.turnsLeft}턴)`)
      .join(', ');
...
function calculateCombatDamage(battle, rawDamage) {
  let helmetReduce =
    (battle.helmetLevel > 0 && battle.helmetDurability > 0)
      ? (battle.helmetLevel * 3)
      : 0;

  let vestReduce =
    (battle.vestLevel > 0 && battle.vestDurability > 0)
      ? (battle.vestLevel * 3)
      : 0;
  ...
  if (
    ...
    battle.helmetDurability > 0
  ) {
    battle.helmetDurability = Math.max(
      0,
      battle.helmetDurability - rand(15, 25)
    );

    if (battle.helmetDurability === 0) {
      battle.helmetLevel = 0;
      armorNotes.push(
        `💥 헬멧 내구도가 0이 되어 파괴되었습니다!`
      );
    }
  }
  ...
  if (
    battle.vestLevel > 0 &&
    battle.vestDurability > 0
  ) {
    battle.vestDurability = Math.max(
      0,
      battle.vestDurability - rand(15, 25)
    );

    if (battle.vestDurability === 0) {
      battle.vestLevel = 0;
      armorNotes.push(
        `💥 조끼 내구도가 0이 되어 파괴되었습니다!`
      );
    }
  }
...
function resolveFarmFight(profile, battle) {
  ...
  if (outcome === 'supply') {
    ...
    if (
      ...
      if (battle.helmetLevel === 0) {
        ...
        resultMessages.push(
          `🛡️ Lv.${battle.helmetLevel} 헬멧으로 업그레이드! (내구도 100%)`
        );
      }
    }
    ...
    if (
      ...
      if (battle.vestLevel === 0) {
        ...
        resultMessages.push(
          `🦺 Lv.${battle.vestLevel} 조끼로 업그레이드! (내구도 100%)`
        );
      }
    }
  }
  ...
  switch (outcome) {
    ...
    case 'kill_single': {
      ...
      const killAssistReward =
        Math.round(
          ((killCount * 500) +
          (assistCount * 250)) * mult
        );
    ...
    case 'kill_multi': {
      ...
      for (let i = 0; i < killCount; i++) {
        ...
        const victimName =
          getRandomSurvivorName();

        if (hitPart === 'head') {
          killDetailLines.push(
            `💥 당신이 헤드샷으로 ${victimName}이(가) 사망했습니다.`
          );
        } else {
          killDetailLines.push(
            `🔫 당신이 ${hitPartName} 부위를 적중시켜 ${victimName}이(가) 사망했습니다.`
          );
        }
      }
...
function applyZoneAttrition(battle) {
  ...
  battle.survivors =
    Math.max(
      1,
      battle.survivors - dec
    );
}

function processEnhance(profile) {
  if (
    profile.enhance === undefined ||
    profile.enhance < 0
  ) {
    profile.enhance = 0;
  }
  ...
  if (
    profile.enhance >= ENHANCE_TABLE.length
  ) {
    const [wName] =
      getWeaponInfo(profile.enhance);

    const stats =
      getEnhanceStats(profile.enhance);

    const detailMsg =
      formatEnhanceStatDiff(
        stats,
        stats
      );
  ...
  if (roll < tableData.success) {
    ...
    resultMsg =
      `🎉 [강화 성공] +${initialEnhance} ➔ +${profile.enhance}\n` +
      `💸 소모 비용: ${won(cost)}\n` +
      `✨ 무기: +${profile.enhance} ${currName}\n` +
      `${detailMsg}`;
...
function processGoldEnhance(
  profile,
  count = 1
) {
  if (!profile.combatLevel) {
  ...
  return {
    text:
      `⚡ 전투력 레벨 +${count} 업그레이드 완료! (현재 Lv.${profile.combatLevel})`,
...
function processUseKey(profile) {
  ...
  if (randRoll < 50) {
    ...
    if (!profile.monthItems) {
      profile.monthItems = 0;
    }
...
function processTurn(
  ...
  if (
    isPlayingBattle &&
    (
      utterance.includes('강화') ||
      utterance.includes('/강화') ||
      utterance.startsWith('금괴강화')
    )
  ) {
    return {
      text:
        `⚠️ 전투 중에는 강화를 진행할 수 없습니다!\n${battleStatusBoard(profile, battle)}`,
  ...
  if (
    utterance.startsWith('/강화') ||
    utterance.startsWith('강화')
  ) {
    const enhanceResult =
      processEnhance(profile);
  ...
  if (
    ...
    let baseExp =
      Math.max(
        100,
        Math.round(
          (battle.accumulatedCash || 0) * 0.007
        )
      );
    ...
    if (!battle.alive) {
      const snap =
        battle.startSnapshot ||
        { cash: profile.cash };

      const deltaCash =
        profile.cash - snap.cash;
  ...
  if (
    ...
    return {
      text:
        `${bodyText}${battleStatusBoard(profile, battle)}`,
