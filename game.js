...
function processImprintReroll(profile) {
  ...
  if (profile.cash < costCash || (profile.gold || 0) < costGold) {
    return { 
      text: `⚠️ 각인 변경 재화가 부족합니다!\n(필요: ${won(costCash)}, 금괴 ${costGold}개 [잠금 슬롯 ${lockedCount}개 적용])\n(보유: ${won(profile.cash)}, 금괴 ${(profile.gold || 0).toLocaleString()}개)` 
    };
  }
  ...
  let newLines = [`🔮 [각인 변경 완료] (소모: ${won(costCash)}, 금괴 ${costGold}개)`];

  unlockedKeys.forEach(k => {
    if (profile.imprintLocks[k]) {
      const opt = profile.imprints[k].options[0];
      newLines.push(`• 각인 ${k} (잠김) : ${opt.name} +${opt.value}${opt.unit}`);
    } else {
      let pool = [...IMPRINT_OPTION_POOL];
      const idx = rand(0, pool.length - 1);
      const optTemplate = pool[idx];
      ...
      newLines.push(`• 각인 ${k} : ${selectedOptions[0].name} +${selectedOptions[0].value}${selectedOptions[0].unit}`);
    }
  });

  newLines.push(``, resourceText(profile));
...
module.exports = {
  ...
  WEAPON_TIERS,
  SHADOW_WEAPON_TIERS,
  STINGER_WEAPON_TIERS,
  SENTINEL_WEAPON_TIERS,
  JOB_WEAPONS,
  ENHANCE_TABLE,
  REFINE_STARS,
  REFINE_TABLE,
  ...
  getRequiredExp,
  getAmplifyInfo,
  getImprintTotalBonus,
  getGoldMultiplier,
  getExpMultiplier,
  getCurrentEnhanceLevel,
  getWeaponInfo,
  getCurrentWeaponName,
  getEnhanceStats,
  formatEnhanceStatDiff,
  formatRefineStatDiff,
  getCombatPower,
  calculatePartDamage,
  addExp,
  createProfile,
